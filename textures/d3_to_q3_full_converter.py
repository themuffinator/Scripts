# d3_to_q3_full_converter.py
# -----------------------------------------------------------------------------
# Doom 3 -> Quake 3 texture & shader converter (Blender bake driver)
# _add / _glow handling aligned with the Q4 converter:
#   - For every "blend add" stage:
#       <base>_add[2..].<ext>   : exact visual copy/convert of source
#       <base>_glow[2..].<ext>  : grayscale->alpha (white=opaque), RGB un-premultiplied
#   - Q3 shader emits one "blend add" stage per _add*
#   - First additive map (if any) is also passed to Blender as --glow to tint the base bake
# -----------------------------------------------------------------------------
...
class D3Material:
    ...
    additive_maps: List[str] = field(default_factory=list)      # normalized tokens (textures/..)
    additive_maps_src: List[str] = field(default_factory=list)  # original tokens (pre-remap)
...
class D3MaterialParser:
    ...
    @staticmethod
    def _norm_token(t: Optional[str]) -> Optional[str]:
        if not t: return t
        t = t.replace("\\", "/").lstrip("/")
        if not t.lower().startswith(("textures/", "models/")):
            t = "textures/" + t
        return t

    def _parse_material_block(self, name: str, block: str, src: Path) -> D3Material:
        mat = D3Material(name=name, source_file=src)
        ...
        if m := re.search(r"qer_?editorimage\s+([^\s}]+)", block, re.I):
            mat.qer_editorimage = self._norm_token(m.group(1))
        ...
        if m := re.search(r"^\s*diffusemap\s+([^\s;}]+)", block, re.MULTILINE | re.I):
            mat.diffuse = self._norm_token(m.group(1))

        if m := re.search(r"^\s*specularmap\s+([^\s;}]+)", block, re.MULTILINE | re.I):
            mat.specular = self._norm_token(m.group(1))

        if m := re.search(r"^\s*bumpmap\s+([^\s;}]+)", block, re.MULTILINE | re.I):
            if 'addnormals' not in m.group(1).lower():
                mat.normal = self._norm_token(m.group(1))

        # Scan stages for diffuse substitute, additive maps, and heightmap stages
        i, n = 0, len(block)
        while i < n:
            ...
            if mm: stage_map = self._norm_token(mm.group(1))

            if not mat.diffuse and blend_mode in ('diffusemap', 'diffuse') and stage_map:
                mat.diffuse = stage_map

            if blend_mode == 'add' and stage_map:
                mat.additive_maps.append(stage_map)
                mat.additive_maps_src.append(stage_map)

            if (not mat.height) and blend_mode == 'bumpmap':
                mh = re.search(r'\bmap\s+heightmap\s*\(\s*([^\s,)]+)\s*,\s*([0-9.+\-]+)\s*\)', stage, re.I)
                if mh:
                    mat.height = self._norm_token(mh.group(1))
                    mat.height_scale = float(mh.group(2))

            i = j

        # addnormals(basemap,heightmap(scale))
        if not mat.normal and not mat.height:
            bn = re.search(
                r"^\s*bumpmap\s+addnormals\s*\(\s*([^\s,()]+)\s*,\s*heightmap\s*\(\s*([^\s,()]+)\s*,\s*([0-9.+\-]+)\s*\)\s*\)",
                block, re.MULTILINE | re.I
            )
            if bn:
                mat.normal = self._norm_token(bn.group(1))
                mat.height = self._norm_token(bn.group(2))
                mat.height_scale = float(bn.group(3))

        if not mat.diffuse and mat.qer_editorimage:
            mat.diffuse = mat.qer_editorimage

        if not mat.diffuse:
            # Fallback: top-level "map" that is not a height/addnormal call
            m = re.search(r"^\s*map\s+([^\s}]+)", block, re.MULTILINE | re.I)
            if m:
                mv = m.group(1)
                if 'heightmap' not in mv.lower() and 'addnormals' not in mv.lower():
                    mat.diffuse = self._norm_token(mv)

        return mat

    def parse_file(self, path: Path) -> Dict[str, D3Material]:
        if not path.exists(): return {}
        text = self.strip_comments(path.read_text(encoding="utf-8", errors="ignore"))
        materials: Dict[str, D3Material] = {}
        ...
        # GUIDE macros (D3 packs also use these patterns)
        for m in self.re_guide.finditer(text):
            ...
            mat = materials.setdefault(mat_name, D3Material(name=mat_name, source_file=path))
            mat.is_guide, mat.guide_macro = True, macro
            if not mat.original_text: mat.original_text = m.group(0).strip()
            
            # inside D3MaterialParser.parse_file(), in the guide-macro loop after computing base/variant:
            # (use whichever arg index your macros actually use; here we try a few likely ones)
            possible_add_args = []
            if len(args) >= 3: possible_add_args.append(args[2])
            if len(args) >= 4: possible_add_args.append(args[3])

            for tok in possible_add_args:
                t = self._norm_token(tok)
                if t and t not in mat.additive_maps:
                    mat.additive_maps.append(t)
                    mat.additive_maps_src.append(t)

            def maps_from(p):
                p = self._norm_token(p)
                mat.diffuse, mat.normal, mat.specular, mat.height, mat.qer_editorimage = f"{p}_d", f"{p}_local", f"{p}_s", f"{p}_h", f"{p}_d"

            def maps_from_noheight(p):
                maps_from(p); mat.height = None

            def variant_diffuse_only(v, b):
                v = self._norm_token(v); b = self._norm_token(b)
                mat.diffuse, mat.qer_editorimage = f"{v}_d", f"{v}_d"
                mat.normal, mat.specular, mat.height = f"{b}_local", f"{b}_s", f"{b}_h"

            def alpha_flags(two_sided=False, no_shadows=False):
                mat.translucent = True
                mat.twoSided = two_sided
                mat.noShadows = no_shadows

            def set_type(i=1):
                if len(args) > i and args[i]:
                    mat.materialType = args[i].strip()

            base = self._norm_token(args[0] if len(args) >= 1 else "")
            variant = self._norm_token(args[1] if len(args) >= 2 else "")

            if macro in ("generic_materialimageshader", "generic_shader", "generic_shader_ed"):
                maps_from(base)
            ...
            elif macro in ("generic_shader2sided",):
                maps_from(base); mat.twoSided = True
            elif macro == "generic_full_noheight":
                maps_from_noheight(base)
            elif macro == "generic_full_noheight_type":
                maps_from_noheight(base); set_type()
            elif macro in ("generic_variant_noheight", "generic_variant_noheight_mi"):
                if variant and base: variant_diffuse_only(variant, base)
                else: maps_from(variant or base)
                mat.height = None
            elif macro == "generic_nonormal":
                maps_from(base); mat.normal = None
            ...
            elif macro in ("generic_alpha", "generic_alpha_ed", "generic_alpha_lv", "generic_alpha_mi"):
                alpha_flags(); maps_from(base)
            elif macro == "generic_alphaglow":
                alpha_flags(); maps_from(base)
                tok = variant or f"{base}_g"
                mat.additive_maps.append(tok); mat.additive_maps_src.append(tok)
            elif macro == "generic_alpha_noshadows":
                alpha_flags(no_shadows=True); maps_from(base)
            elif macro == "generic_alphanoshadow2s":
                alpha_flags(two_sided=True, no_shadows=True); maps_from(base)
            elif macro in ("generic_shader2sidedalpha", "generic_localvalpha2"):
                maps_from(base); alpha_flags(two_sided=True)
            ...
            elif macro == "generic_shader2sidedalpha_type":
                maps_from(base); alpha_flags(two_sided=True); set_type()
            elif macro in ("generic_glow", "generic_glow_mi"):
                maps_from(base)
                tok = variant or f"{base}_g"
                mat.additive_maps.append(tok); mat.additive_maps_src.append(tok)
            elif macro in ("generic_terminal_replaceglow", "generic_terminal_replaceglow2"):
                maps_from(base)
                tok = variant or f"{base}_add"
                mat.additive_maps.append(tok); mat.additive_maps_src.append(tok)
            elif macro == "generic_typeshader":
                maps_from(base); set_type()
            elif macro == "generic_localvariant_typeshader":
                maps_from(variant or base); set_type(2)
            elif macro == "generic_alpha_type":
                maps_from(base); alpha_flags(); set_type()
...
class NameManager:
    ...
    def _build_directory_map(self, original_paths: list) -> dict:
        ...
        for original_dir in sorted(list(first_level_dirs)):
            # Keep category mapping style consistent with your Q4 pipeline
            if original_dir.lower().startswith("common_"):
                dir_map[original_dir] = f"d3_{original_dir[7:]}"
            else:
                dir_map[original_dir] = f"d3_{original_dir}"
        return dir_map
    ...
    def get_new_name(self, original_full_path: str) -> str:
        ...
        new_dir = self.dir_map.get(original_first_dir, f"d3_{original_first_dir}")
        ...
        p = p.replace("support", "supp").replace("nonsolid", "ns").replace("texture", "tex").replace("collision", "coll")
...
def apply_suffix_indexed(q3_noext: str, desired: str, idx: int) -> str:
    # idx=0 => _add / _glow; idx=1 => _add2 / _glow2, etc.
    if idx == 0:
        return apply_suffix(q3_noext, desired)
    parts = q3_noext.replace("\\", "/").split("/")
    base = parts[-1]
    base = _SUFFIX_RE.sub("", base)
    parts[-1] = f"{base}_{desired}{idx+1}"
    return "/".join(parts)
...
def black_to_transparency_glow(in_path: Path, out_path: Path, log_path: Optional[Path] = None) -> bool:
    """
    Make black transparent (alpha = luminance), un-premultiply RGB to avoid dark fringes.
    """
    try:
        ...
        out.save(out_path)
        if log_path:
            write_log_lines(log_path, [f"[GLOW_BLACK2ALPHA] {in_path} -> {out_path}"])
        return True
    except Exception as e:
        if log_path:
            write_log_lines(log_path, [f"[ERROR] black_to_transparency_glow failed: {e}"])
        return False

# =============================== BLENDER & SHADER ============================

def call_blender_bake(cfg: Config, mat: D3Material, d_path: Path, out_path: Path,
                      n_path, s_path, h_path, log_path, glow_path=None) -> int:
    args = [
    ...
    if glow_path: args.extend(["--glow", str(glow_path)])
...
def make_q3_shader(mat: D3Material, out_format: str) -> str:
    ...
    # One additive stage per _add*, maintaining order
    for i, _ in enumerate(mat.additive_maps):
        add_noext = apply_suffix_indexed(q3_path, "add", i)
        lines.extend([
            "\t{",
...
def main():
    ...
    parser = D3MaterialParser(verbose=cfg.verbose)

    # Load all .mtr from base_root/materials (and subdirs)
    all_mats: Dict[str, D3Material] = {}
    ...
    # Only textures/ or models/mapobjects/ are interesting for Q3
    materials_to_process: Dict[str, D3Material] = {}
    ...
    print(f"[INFO] Filtered to {len(materials_to_process)} materials for processing.")

    # Name restructuring
    name_manager = NameManager(list(materials_to_process.keys()))

    parser_failures = [
        name for name, mat in materials_to_process.items()
    ...
    if parser_failures:
        ...
        print(f"[WARN] {len(parser_failures)} materials missing diffuse. See {failure_log_path}")
        with open(failure_log_path, "w", encoding="utf-8") as f:
            f.write(f"# {len(parser_failures)} materials lacked a diffuse token:\n\n")
    ...
    processed_materials: Dict[str, Tuple[str, D3Material]] = {}
    ...
    for original_name, mat in materials_to_process.items():
        ...
        # keep additive_maps_src; do not rename for resolution — only map output names
    ...
    for new_name, (original_name, mat) in sorted(processed_materials.items()):
        ...
        try:
            ...
            if not d_path:
                if cfg.verbose: print(f"[SKIP] {original_name}: cannot resolve '{mat.diffuse}'.")
                continue

            n_path = index.resolve(mat.normal)
            s_path = index.resolve(mat.specular)
            h_path = index.resolve(mat.height)

            # resolve additive sources
            glow_src_paths: List[Path] = []
            for tok in mat.additive_maps_src:
                p = index.resolve(tok)
                if p: glow_src_paths.append(p)

            deps = [p for p in [d_path, n_path, s_path, h_path] + glow_src_paths if p]
            if not should_rebuild(out_img, deps if deps else [d_path], cfg):
                ...
                first_glow = glow_src_paths[0] if glow_src_paths else None
                if not any([n_path, s_path, h_path]) and not first_glow or mat.force_copy:
                    shutil.copy2(d_path, out_img)
                else:
                    rc = call_blender_bake(cfg, mat, d_path, out_img, n_path, s_path, h_path, log_path, glow_path=first_glow)
                    if rc != 0:
                        print(f"[ERROR] Blender failed for {original_name} (rc={rc}). Falling back to copy.")
                        shutil.copy2(d_path, out_img)

            # Emit _add/_glow for every additive stage
            if glow_src_paths:
                q3_noext = f"textures/{new_name}"
                for i, add_src in enumerate(glow_src_paths):
                    add_noext_i  = apply_suffix_indexed(q3_noext, "add",  i)
                    glow_noext_i = apply_suffix_indexed(q3_noext, "glow", i)
                    add_out  = cfg.dst_base / (add_noext_i  + f".{output_format}")
                    glow_out = cfg.dst_base / (glow_noext_i + f".{output_format}")

                    # _add : exact visual copy (convert container if needed)
                    if add_out.suffix.lower() == add_src.suffix.lower():
                        ...
                        if not blender_convert(cfg, add_src, add_out, log_path):
                            try: shutil.copy2(add_src, add_out)
                                ...
                                write_log_lines(log_path, [f"[WARN] _add copy failed: {e}"])

                    # _glow : black->transparent (alpha=luma), un-premultiply
            ...
            if log_path:
                write_log_lines(log_path, [f"FATAL ERROR: {e}", traceback.format_exc()])
    ...
    for source_name, blocks in sorted(shaders_by_file.items()):
        shader_filename = f"{source_name}.shader"
